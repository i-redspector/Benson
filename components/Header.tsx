...
const Header: React.FC = () => {
  ...
  return (
    <>
    <div className="fixed top-0 left-0 w-full h-[2px] bg-white/10 z-[60]">
        <div 
    ...
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <button className="md:hidden text-white z-50 p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          {mobileMenuOpen ? <X className="text-bg-gold w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>

        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="fixed inset-0 bg-black flex flex-col items-center justify-center gap-8 z-40 animate-fade-in">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-bg-gold/10 to-transparent opacity-30 pointer-events-none"></div>
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                className="text-2xl font-serif text-white hover:text-bg-gold transition-colors tracking-wide"
              >
                {link.name}
              </a>
            ))}
            <div className="flex gap-5 mt-8">
             ...
             <a href="#contact" onClick={(e) => handleNavClick(e, '#contact')} className="mt-8 bg-bg-gold text-black px-10 py-4 rounded-sm uppercase text-xs font-bold tracking-[0.2em]">
                Client Login
             </a>
          </div>
        )}
